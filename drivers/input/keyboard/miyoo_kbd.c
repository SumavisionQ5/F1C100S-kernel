...
static void scan_handler(unsigned long unused)
{
  ...
  if(val & MY_R && !non_hotkey_first) {
      ...
      if(!hotkey_down) {
		static char * shutdown4_argv[] = { "/bin/sh", "-c", "/bin/kill -9 $(/bin/ps -al | /bin/grep \"/mnt/\" | /bin/grep -v \"/kernel/\" | /usr/bin/tr -s [:blank:] | /usr/bin/cut -d \" \" -f 2) ; /bin/sleep 0.1 ; /bin/sync ; /bin/swapoff -a ; /sbin/poweroff",  NULL };
        ...
        hotkey_down = true;
		// Quick Shutdown is more usefull than just shutting down the applicaiton.
      }
			hotkey_actioned = true;
